...
- **البيانات تتراكم (مهم):** الـ DB لا يطابق الشيت تلقائيًا — الـ importer يحذف فقط مفاتيح group+date الموجودة في الملف الجديد، فتبقى صفوف قديمة (~50% من جدول lectures مش في الشيت الحالي). نوعان: (أ) `مؤكدة` قديمة = **تاريخ حقيقي محفوظ عمدًا** (مجموعات منتهية/معاد تسميتها)؛ (ب) `مجدولة` قديمة = **شبح** (مش في الشيت، تضخّم أي عدّ يشملها). أي تقرير يَعُدّ من `lectures` لازم يكون: `مؤكدة` فقط (للمُسلّم) **أو** `COUNT(DISTINCT canonical)` + rename-aware. التقارير المبنية على `remarks`/`absent_students`/أعمدة `batches` المتزامنة غير متأثرة.
