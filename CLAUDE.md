...
- **حالة `غير مؤكدة`:** المحجوز يَعُدّ المحاضرات حتى لو `status='غير مؤكدة'` (لا فلتر حالة) — فمدرّب شيفته منتهٍ ولديه محاضرات مجدولة/غير مؤكدة لاحقة تظهر نسبته متضخّمة (مثل Nashwa 265%): هذه **مشكلة بيانات** (تاريخ شيفت قديم أو اسمه لا يزال في عمود المدرّب لمجموعات مستمرة)، لا خطأ حساب — يصلّحها الـ Owner في فريق العمل/الشيت.
