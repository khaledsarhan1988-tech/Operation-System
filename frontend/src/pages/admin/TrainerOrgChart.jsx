...
import { Network, Layers, Users, BookOpen, GraduationCap, AlertCircle } from 'lucide-react';
...
// group / student / lecture counts. No leader row, no capacity meter, no
// simulation — trainers aren't coordinators, so those concepts don't apply.
function TrainerColumnCard({ section }) {
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHero
        ...
        subtitle="محاضرو المحاضرات الأساسية موزّعين على الأقسام (عام / شبه خاص / خاص)"
        icon={Network}
      ...
      {!isLoading && !isError && sections.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {sections.map((s) => (
            <TrainerColumnCard key={s.key} section={s} />
          ))}
        </div>
      )}
