...
export default function AgentCodeProblems() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await qc.invalidateQueries({ queryKey: ['agent-code-problems'] });
      setEditKey(null);
    } catch (e) {
           ...
           rows.map((p, i) => {
             ...
             return (
               <tr key={i} className={`border-b border-gray-50 hover:bg-gray-50/60 transition-colors ${rowBg}`}>
                 <td className="px-4 py-3 font-semibold text-gray-900 text-xs" style={{ maxWidth: '240px', wordBreak: 'break-word' }}>{p.group_name}</td>
    ...
    <div className="space-y-0 animate-fadeIn" dir="rtl">
      ...
      <div className="px-6 pt-5 pb-4 border-b border-gray-100 bg-gradient-to-l from-slate-50 to-white rounded-t-2xl">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {hasFilters && (
            <button
              onClick={() => { setSearch(''); setFSection('all'); setFProbType(''); setFStatus(''); }}
