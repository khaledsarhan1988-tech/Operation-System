...
// GET /api/leader/absent-report?group=&status=&from=&to=&page=&limit=
router.get('/absent-report', (req, res) => {
  const { group, status, from, to, page = 1, limit = 50 } = req.query;
  const conditions = [];
  const params = [];
...
// GET /api/leader/performance?from=&to=
